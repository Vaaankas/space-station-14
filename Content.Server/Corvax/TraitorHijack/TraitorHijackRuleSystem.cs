...
public sealed class TraitorHijackRuleSystem : GameRuleSystem<TraitorHijackRuleComponent>
{
    ...
    protected override void ActiveTick(EntityUid uid, TraitorHijackRuleComponent component, GameRuleComponent gameRule, float frameTime)
    {
        base.ActiveTick(uid, component, gameRule, frameTime);

        if (component.SelectionStatus == TraitorHijackRuleComponent.SelectionState.ReadyToSelect && _gameTiming.CurTime > component.AnnounceAt)
            DoTraitorStart(component);
    }

    private void OnStartAttempt(RoundStartAttemptEvent ev)
    {
        var query = EntityQueryEnumerator<TraitorHijackRuleComponent, GameRuleComponent>();
    ...
    private void MakeCodewords(TraitorHijackRuleComponent component)
    {
    ...
    private void DoTraitorStart(TraitorHijackRuleComponent component)
    {
        ...
        component.SelectionStatus = TraitorHijackRuleComponent.SelectionState.SelectionMade;
    }
    ...
    private void OnPlayersSpawned(RulePlayerJobsAssignedEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out var traitor, out var gameRule))
        {
            ...
            traitor.SelectionStatus = TraitorHijackRuleComponent.SelectionState.ReadyToSelect;
        }
    }

    public List<IPlayerSession> FindPotentialTraitors(in Dictionary<IPlayerSession, HumanoidCharacterProfile> candidates, TraitorHijackRuleComponent component)
    {
    ...
    {
        var traitorRule = EntityQuery<TraitorHijackRuleComponent>().FirstOrDefault();
        if (traitorRule == null)
        {
            //todo fuck me this shit is awful
            //no i wont fuck you, erp is against rules
            GameTicker.StartGameRule("Traitor", out var ruleEntity);
            traitorRule = Comp<TraitorHijackRuleComponent>(ruleEntity);
        }
        ...
        //give traitors their objective
        var objective = _objectivesManager.GetRandomObjective(traitorRole.Mind, "TraitorObjectiveGroupHijack");
        if (objective != null) {
            traitorRole.Mind.TryAddObjective(objective);
        };
    ...
    private void HandleLatejoin(PlayerSpawnCompleteEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out var traitor, out var gameRule))
        {
            ...
            if (traitor.SelectionStatus < TraitorHijackRuleComponent.SelectionState.SelectionMade)
            {
    ...
    public List<TraitorRole> GetOtherTraitorsAliveAndConnected(Mind.Mind ourMind)
    {
        ...
        foreach (var traitor in EntityQuery<TraitorHijackRuleComponent>())
        {
    ...
    public List<TraitorRole> GetOtherTraitorsAliveAndConnected(Mind.Mind ourMind, TraitorHijackRuleComponent component)
    {
